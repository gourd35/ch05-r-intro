...
rain <- function(chance_of_rain, I_saw_raindrops, my_shoes_are_wet, my_ta_is_soaked){
  if (I_saw_raindrops) {
    chance_of_rain <- 100
  } else {
    if (my_shoes_are_wet) {
...
rainChance <- rain(0, F,F,T)
cat("The chance of rain in Seattle is",rainChance,"percent\n")
